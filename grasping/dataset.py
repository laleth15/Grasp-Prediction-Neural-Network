...
class GraspDataset(Dataset):
    ...
    def transform_grasp(self, img: Tensor, action: np.ndarray) -> Tuple[Tensor, np.ndarray]:
        ...
        action:
            ...
            the image (height dimension), py specifies the column in the image (width dimension),
        ...
        '''
        # Randomly choose a rotation angle from 0, 90, 180, or 270 degrees
        rot_angles = [0, 90, 180, 270]
        random = np.random.choice(rot_angles)
        rot_deg = int(random)
        ...
        new_action = (new_px, new_py, new_id)

        new_img = TF.rotate(img=img, angle = rot_deg)

        return new_img, new_action
