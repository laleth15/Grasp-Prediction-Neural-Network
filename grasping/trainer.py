...
class MobileUNet(nn.Module):
    def __init__(self, pretrained: bool=True, num_rots: int=2) -> None:
        ...
        self.sequence1 = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Upsample(align_corners=True, scale_factor=8, mode='bilinear')
        )

        self.sequence2 = nn.Sequential(
            nn.Upsample(align_corners=True, scale_factor=4, mode='bilinear'),
    ...
    def forward(self, img: Tensor) -> Tensor:
        ...
        # x_wide = self.flatten(x_wide)
        # x_narrow = self.flatten(x_narrow)
        
        # x_wide = torch.permute(x_wide, (1,2,3,0))
        # x_narrow = torch.permute(x_narrow, (1,2,3,0))
        # print(x_wide.size())

        x_narrow = self.sequence1(x_narrow)
        x_wide = self.sequence2(x_wide)
